...
def get_optimizer(model: nn.Module, **kargs):
    ...
    base_weight_decay = float(kargs.pop('weight_decay', 0.0))
    lr = kargs.pop('lr', None)
    betas = kargs.pop('betas', None)
    eps = kargs.pop('eps', None)
    ...
    for module_name, module in model.named_modules():
        for param_name, param in module.named_parameters(recurse=False):
            ...
            is_norm = isinstance(module, (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d, nn.LayerNorm, nn.GroupNorm, nn.InstanceNorm1d, nn.InstanceNorm2d, nn.InstanceNorm3d))
    ...
    if len(decay_params) > 0:
        param_groups.append({'params': decay_params, 'weight_decay': base_weight_decay})
    if len(no_decay_params) > 0:
        param_groups.append({'params': no_decay_params, 'weight_decay': 0.0})

    if not param_groups:
        param_groups = [{'params': [p for p in model.parameters() if p.requires_grad], 'weight_decay': base_weight_decay}]
    ...
    if eps is not None:
        optim_kwargs['eps'] = eps

    return OptimCls(param_groups, **optim_kwargs)
...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model):
        ...
        if self.best_score is None:
            ...
            self.best_score = score
            self.save_checkpoint(val_loss, model)
            self.recent_counter_saver = self.counter
            self.counter = 0
